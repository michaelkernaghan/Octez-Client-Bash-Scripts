...
# Calculate the start level. We are interested in the last 50 blocks, so we subtract 50 from the head level.
start_level=$((head_level - 50))
...
for ((level = head_level; level >= start_level; level--)); do
    ...
    transactions=$(echo "$operations" | grep -A18 "$baker_address" --no-group-separator)
...
# Print the summary
echo -e "${COLORS[GREEN]}Summary: Transactions were found in $transaction_blocks out of 100 checked blocks.${COLORS[NC]}"
